...
RUNTIME = 30
RETRIES = 1
BASE_DIR = r"/home/bdaviv/tmp/results/Arthur-compare"
...
def create_vms():
    ...
    vm_list.append(deepcopy(e1000))

    # Checksum
    vm_list.append(deepcopy(vm_list[-1]))
    ...
    # interrupts
    vm_list.append(deepcopy(vm_list[-1]))
    vm_list[-1].e1000_options["NG_interrupt_mul"] = 10
    vm_list[-1].e1000_options["NG_interrupt_mode"] = 0
    ...
    pairs.extend(pairwise(vm_list))

    # virtio, Again
    virtio_drop = deepcopy(virtio)
    virtio_drop.e1000_options["NG_drop_packet"] = "on"
    virtio_drop.name = "Virtio-Packet_Drop"

    pairs.append((virtio_drop, vm_list[-1]))
    pairs.append((vm_list[-1], e1000))
    pairs.append((virtio_drop, vm_list[-2]))
