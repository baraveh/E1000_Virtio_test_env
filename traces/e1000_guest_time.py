...
def main(directory=None):
    ...
    vm = QemuE1000Max(disk_path=r"/homes/bdaviv/repos/e1000-improv/vms/ubuntu.img",
                                   guest_ip="10.10.0.43",
                                   host_ip="10.10.0.44")
...
def parse_results(directory):
    ...
    for event1, event2 in zip(events[:-1], events[1:]):
        ...
        if event1.event == 'kvm_entry':
            if event2.event in ('e1000_post_mem_op', 'e1000_post_set_tdt', 'irq_handler_entry'):
                entry_event = event1 # TODO: fix it to event2
            else:
                entry_event = event1
        # on exit, check the event before
        elif event2.event == 'kvm_exit':
            if event1.event in ('e1000_pre_mem_op', 'e1000_set_tdt', 'cpu_idle'):
                exit_event = event1
            else:
                exit_event = event2
